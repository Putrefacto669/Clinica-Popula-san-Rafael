...
def conectar_phpmyadmin():
    ...
    try:
        conn = pyodbc.connect(
            "DRIVER={MySQL ODBC 9.5.0 ANSI Driver};"  # o el driver que tengas instalado
            "SERVER=localhost;"
...
def enviar_correo_paciente(nombre, correo_destinatario, mensaje):
    """
    Enviar correo electrónico a paciente
    """
    try:
        # CONFIGURA TUS CREDENCIALES AQUÍ
        remitente = "lopezurbina2018@gmail.com"
        password = "uynt mkho qwbf xtyp"  # Contraseña de aplicación Gmail
        
        # Crear mensaje
        msg = MIMEMultipart()
        msg["Subject"] = "Información sobre su cita médica - Clínica San Rafael"
        msg["From"] = remitente
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
                <div style="background: #0A64A4; color: white; padding: 20px; border-radius: 8px 8px 0 0; text-align: center;">
                    <h1 style="margin: 0;">🏥 Clínica San Rafael</h1>
                    <p style="margin: 5px 0 0 0; opacity: 0.9;">Sistema de Gestión Médica</p>
                </div>
                
                <div style="padding: 25px;">
                    <h2 style="color: #0A64A4;">Estimado/a {nombre},</h2>
                    ...
                    <div style="margin-top: 25px; padding-top: 20px; border-top: 1px solid #eee;">
                        <p style="margin-bottom: 5px;"><strong>Saludos cordiales,</strong></p>
                        <p style="margin: 0; color: #0A64A4;"><strong>Equipo Médico - Clínica San Rafael</strong></p>
                        <p style="margin: 5px 0;">📞 Teléfono: +505 1234-5678</p>
                        <p style="margin: 0;">📍 Managua, Nicaragua</p>
                    </div>
                </div>
            </div>
        </body>
        ...
        guardar_correo_db(nombre, correo_destinatario, mensaje)
...
def guardar_correo_db(paciente, email, mensaje):
    ...
    try:
        ...
        conn = pyodbc.connect(
            "DRIVER={MySQL ODBC 9.5.0 ANSI Driver};"
            "SERVER=localhost;"
...
ttk.Button(frame_controles_citas, text="📧 Enviar Correo", command=abrir_formulario_correos).grid(row=0, column=5, padx=5, pady=4)
...
def inicializar_aplicacion_completa():
    ...
    # INICIALIZACIÓN DEL MÓDULO DE CITAS (NUEVO)
    try:
